...
const SetNewPassword = () => {
  ...
  React.useEffect(() => {
    if (!email || !code) {
      Swal.fire({ icon: "error", title: "Invalid Access", text: "Please start from the forgot password page." });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/user/reset-password`, {
        ...
        body: JSON.stringify({ email, code, newPassword: formData.newPassword }),
      });
